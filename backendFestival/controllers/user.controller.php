...
class UserController {
  ...
  public function listarUser() {
    ...
    if(IDUSER) {
      $eval = "SELECT id,nombre,apellidos,email,telefono,imgSrc,pais,sexo FROM users";
  ...
  public function leerPerfil() {
    if(IDUSER) {
      $eval = "SELECT nombre,apellidos,email,telefono,imgSrc,pais,sexo FROM users WHERE id=?";
      $peticion = $this->db->prepare($eval);
  ...
  public function registrarUser() {
    ...
    if(!isset($user->pais)) $user->pais = null;
    ...
    if(!$resultado) {
      ...
      $eval = "INSERT INTO users (nombre,apellidos,password,email,telefono,pais,sexo) VALUES (?,?,?,?,?,?,?)";
      $peticion = $this->db->prepare($eval);
      $peticion->execute([
        $user->nombre,$user->apellidos,$password,$user->email,$user->telefono,$user->pais,$user->sexo
      ]);
  ...
  public function editarUser() {
    if(IDUSER) {
      ...
      $peticion = $this->db->prepare("SELECT nombre,apellidos,email,telefono,pais,sexo FROM users WHERE id=?");
      $peticion->execute([IDUSER]);
      $resultado = $peticion->fetchObject();
      ...
      $nPais = isset($user->pais) ? $user->pais : $resultado->pais;
      ...
      if(isset($user->password) && (strlen($user->password))){
        ...
        $eval = "UPDATE users SET nombre=?,apellidos=?,password=?,email=?,telefono=?,pais=?, sexo=? WHERE id=?";
        $peticion = $this->db->prepare($eval);
        $peticion->execute([$nNombre,$nApellidos,$nPassword,$nEmail,$nTelefono,$nSexo,$nPais,IDUSER]);
      } else {
        $eval = "UPDATE users SET nombre=?,apellidos=?,email=?,telefono=?,pais=?,sexo=? WHERE id=?";
        $peticion = $this->db->prepare($eval);
        $peticion->execute([$nNombre,$nApellidos,$nEmail,$nTelefono,$nSexo,$nPais,IDUSER]);        
      }
      http_response_code(201);
